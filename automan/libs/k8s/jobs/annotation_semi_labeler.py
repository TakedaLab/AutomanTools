...
class AnnotationSemiLabeler(BaseJob):
    IMAGE_NAME = 'automan-annotation-semi-labeler'

    # TODO: automan_server_info
    def __init__(
            self, storage_type, storage_config, automan_config,
            semi_labeling_config, k8s_config_path=None, ros_distrib='kinetic'):
        super(AnnotationSemiLabeler, self).__init__(k8s_config_path)
        ...
        if storage_type == 'LOCAL_NFS':
            ...
            self.semi_labeling_info = json.dumps(semi_labeling_config, separators=(',', ':'))
        else:
    ...
    def __get_containers(self):
        ...
        args = ['python', '/app/bin/automan_annotation_semi_labeler.py',
                '--automan_info', self.automan_info, '--semi_labeling_info', self.semi_labeling_info]
        system_usage = {}
        containers = [
